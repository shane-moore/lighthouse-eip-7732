...
use execution_layer::http::{ENGINE_GET_PAYLOAD_V6, ENGINE_NEW_PAYLOAD_V6};
...
pub enum GloasReadiness {
    ...
    Ready,
    /// We are connected to an execution engine which doesn't support the V6 engine api methods
    V6MethodsNotSupported { error: String },
...
impl fmt::Display for GloasReadiness {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GloasReadiness::V6MethodsNotSupported { error } => write!(
                f,
                ...
                .await
            {
                ...
                Ok(capabilities) => {
                    ...
                    if !capabilities.get_payload_v6 {
                        ...
                        missing_methods.push_str(ENGINE_GET_PAYLOAD_V6);
                        all_good = false;
                    }

                    if !capabilities.new_payload_v6 {
                        missing_methods.push(' ');
                        missing_methods.push_str(ENGINE_NEW_PAYLOAD_V6);
                        all_good = false;
                    }
                    ...
                    if all_good {
                        ...
                        GloasReadiness::V6MethodsNotSupported {
